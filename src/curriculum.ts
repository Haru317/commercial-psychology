...
export const days: Day[] = [
  {
    day: 1, stage: 1, title: "依頼された問題が、本当に解くべき問題か判定する", capability: "経営者の依頼を鵜呑みにせず、価値毀損・意思決定・診断範囲を結ぶDecision Mandateを作る",
    question: "CEOの「需要が消えた。広告を増やしたい」に対し、何を調べ、何が判明したら、どの経営判断を変えるのか。",
    why: "数字の低下を見つけても、それが利益・キャッシュ・戦略顧客・能力形成を毀損しないなら、解く価値のない問題かもしれない。Day 1では測定より先に、診断へ経営資源を使う妥当性を審査する。",
    theory: { name: "Decision Mandate & Problem Validity Gate / 診断指示書と問題妥当性審査", plain: "症状を測る前に、誰のどの意思決定のために、何の価値毀損を、どの範囲・期限で診断するかを固定する。", can: "依頼者の不安と、会社が本当に変えるべき経営結果を分離できる。", limit: "原因や解決策はまだ確定しない。ここでは『調べる価値があるか』と『何が分かれば決められるか』だけを定める。", misuse: "売上低下を自動的に問題とみなす／KPIを細かくしただけで診断課題ができたと思う／広告増額を前提に原因を探す。" },
    case: { name: "Peloton FY2022 · CEO緊急診断", role: "需要・収益モデル・在庫問題を分け、30日で経営判断を返す", scene: "FY2022決算後、CEOは『コロナ特需が終わり需要が消えた。広告を増やせば戻る』と主張。あなたはPartnerから、広告予算の承認前に診断指示書を作るよう求められた。", facts: ["総売上高はFY2021の約40.22億ドルからFY2022の約35.82億ドルへ11%減。", "Connected Fitness Products売上は約31.50億ドルから約21.88億ドルへ31%減。", "Subscription売上は約8.72億ドルから約13.94億ドルへ60%増。Connected Fitness Subscriptionsは約233万から約297万へ27%増。", "期末在庫は約5.22億ドルから約12.17億ドルへ増加。売上減は『顧客需要』だけでなく、製品・継続課金・在庫・コスト構造の複合問題。"], reasoning: ["01｜依頼を決定へ戻す：問うべきは『広告を増やすか』ではなく、90日間の資金を新規獲得・既存会員価値・在庫圧縮のどこへ配分するか。", "02｜価値毀損を確認する：全社売上▲4.40億ドルだけでは不十分。製品売上▲9.62億ドル、Subscription売上＋5.22億ドル、在庫＋6.95億ドルを分けると、事業全体の需要消滅という説明は成立しない。", "03｜商業システムを描く：広告→サイト訪問→購入→配送・設置→利用→継続課金という流れに、価格、CAC、在庫、供給能力、解約を接続する。", "04｜Validity Gateを通す：製品獲得の悪化がContribution Marginとキャッシュを毀損し、放置時の損失が診断費用を上回るなら優先課題。会員基盤の成長は別の価値源として保護する。", "05｜診断指示書へ固定する：30日で、新規機器需要の減少量を価格・流入・転換・供給要因へ分解し、広告増額／在庫圧縮／会員収益化への資源配分案を経営会議へ提示する。"] },
    drills: [
      { prompt: "この依頼の背後にある「30日後に変える経営判断」を一文で書く。", avoid: "『売上低下の原因を調べる』で止める／広告増額を既定路線にする。", hint: "誰が・いつまでに・どの選択肢へ・何を配分する判断か。", model: "CEOとCFOが30日後の経営会議で、次四半期の成長予算を①新規Connected Fitness獲得、②既存会員の継続価値向上、③在庫圧縮のどこへ何％配分するか決める。", why: "分析の利用者・期限・選択肢・希少資源があり、調査結果が実際の意思決定を変える。", mistake: "『Pelotonのブランド力を改善する』のように、決定者・期限・配分対象がない。" },
      { prompt: "4つの事実を使い、『需要消滅』という問題設定を審査する。", avoid: "全社売上▲11%だけで問題を確定する／会員増だけで安心する。", hint: "製品、Subscription、会員基盤、在庫を同じ前年差で分解する。", model: "全社売上は▲4.40億ドルだが、内訳は製品▲9.62億ドル、Subscription＋5.22億ドルで、会員基盤も＋27%。一方、在庫は＋6.95億ドルである。したがって『需要全体の消滅』は棄却し、①新規機器獲得の悪化、②成長する継続収益の価値、③需要予測と在庫の不整合を別問題として優先度評価する。", why: "集計値を経済構造の異なる収益源へ分解し、経営者の説明がデータ全体と整合するか検査している。", mistake: "売上・顧客数・在庫を一つの『人気』という心理説明へまとめる。" },
      { prompt: "診断範囲・除外範囲・追加依頼資料を含むMandateを完成させる。", avoid: "会社全体を無期限に調べる／公開10-Kだけで原因まで断定する。", hint: "30日で識別でき、結論により予算配分が変わる境界を置く。", model: "対象は米国Connected FitnessのFY2022新規獲得と在庫ミスマッチ。価格・チャネル別流入・購入転換・CAC・返品・配送・SKU別在庫を週次で分解し、既存会員の解約原因と海外市場は別ワークストリームとする。Growth、Finance、Supply Chainへ、週次ファネル、広告費、粗利、在庫エイジング、需要予測、返品理由の提出を5営業日以内に依頼する。", why: "原因候補を識別できる資料、社内の保有部署、期限、意図的な除外範囲まで実務指示に落ちている。", mistake: "『顧客理解を深めるためアンケートする』のように、既存社内データより先に新規調査へ飛ぶ。" },
    ],
    independent: { name: "地方美容クリニック · Problem Validity Gate", scene: "総予約数と営業利益は横ばい。受付は『20代に選ばれなくなった』と危機感を示すが、院は高単価のたるみ治療へ設備投資し、50–60代の継続率と客単価が伸びている。近隣には若年向け低価格院が3院開業した。", prompt: "20代予約減を本当に優先課題とするか判定し、経営判断・必要資料・診断範囲を示してください。", model: "現時点では優先課題と確定しない。院長が決めるべきことは、限られた広告・施術枠を20代再獲得と50–60代高単価治療のどちらへ配分するかである。年齢×施術別の売上、限界利益、再来率、稼働率、設備回収計画をFinance／受付／マーケティングから取得し、20代減少による将来LTV・紹介・稼働への損失が、成熟層成長の機会費用を上回る場合のみ診断対象にする。" },
    transfer: "自分の案件の『困りごと』を、①変える経営判断、②価値毀損、③Problem Validity Gate、④範囲・除外、⑤誰に何の資料をいつまでに依頼するか、の5項目でDecision Mandateにする。",
    rubric: { pass: "決定者・期限・選択肢・商業結果があり、依頼者の原因説明を確定事実にしていない。", standard: "問題を解く価値、診断範囲、除外範囲、追加資料と保有部署が明確。", pro: "結果がAなら資源配分X、BならYと、証拠から意思決定が変わる条件まで書ける。", revise: "『売れない原因を調べる』を消し、何を守る／伸ばすために誰が何を決めるかから書き直す。" },
    output: "Diagnostic Mandate v1", next: "問題を固定しても、見聞きした情報に解釈が混ざっていれば誤診する。Day2で証拠を分離する。", sources: [peloton],
  },
  {
    day: 2, stage: 1, title: "証拠と物語を分離する", capability: "Day 1の診断指示書に入った主張を、意味分類・証拠状態・出所・適用範囲へ分解し、原因を断定せず次に取る証拠を決める",
    question: "CEO・公開資料・顧客事例が語る別々の物語を、同じFact欄へ混ぜず、経営判断に使えるEvidence Logへ変えるにはどうするか。",
    why: "数字、発言、計算、会社側の原因説明を同列に置くと、強い数値に弱い物語が便乗する。Day 2ではDay 1のMandateを入力に、何が確認済みで、何が矛盾し、何がまだ未確認かを固定する。",
    theory: { name: "Evidence Separation & Ladder of Inference / 証拠分離と推論の梯子", plain: "記録を①観測・記録値、②当事者の発言、③元データから再現可能な計算、④解釈、⑤因果仮説、⑥将来予測に分け、別列でconfirmed／contradicted／unverifiedを付ける。", can: "同じ資料内の事実と会社側ナラティブを分離し、未確認の推論だけを次の調査対象にできる。", limit: "分類は証拠の信頼度や原因を確定しない。出所・測定・利害の重みづけはDay 3で行う。", misuse: "発言を嘘として捨てる／数字なら原因まで証明すると考える／責任の所在をDay 2で決める。" },
    case: { name: "Zoom FY2019 S-1 · Growth Narrative Review", role: "Day 1で固定した『Enterprise成長投資を続けるか』という判断のため、成長物語を検証可能なEvidence Logへ変える", scene: "CEOは『摩擦の少ない体験と口コミがEnterprise成長を生んだ』と説明する。Partnerは、S-1、VMware顧客事例、社内ファネルが示す範囲を分け、まだ何が分からないかをCFOへ返すよう指示した。", facts: ["S-1記載のFY2019売上は約3.305億ドル、純利益は約760万ドル。これは全社結果であり、成長理由そのものではない。", "S-1の顧客事例ではVMware約19,000人が利用し、3か月で4,100万分超の会議を実施。これは一社の利用規模であり、市場全体の購入理由や収益性を示さない。", "『frictionless』『viral』は会社側の解釈・因果説明。無料利用、営業活動、既存顧客拡張、価格、競合移行の寄与率は公開資料だけでは未確認。", "必要な次証拠は、acquisition source、free→paid conversion、seat expansion、sales-assisted／self-serve別CAC・継続率を同一コホートで結ぶ社内データ。"], reasoning: ["01｜Day 1を引き継ぐ：誰のどの判断に使うEvidence Logかを書き、無関係な事実収集を止める。", "02｜意味を分類する：数値、引用、計算、解釈、因果仮説、予測を別行にする。", "03｜状態を付ける：confirmedは出所で直接確認、contradictedは同じ範囲の証拠と矛盾、unverifiedは判断材料が不足。", "04｜適用範囲を切る：VMware一社の利用を、全顧客の購入理由や利益へ一般化しない。", "05｜次証拠を設計する：未確認の因果仮説ごとに、結果が異なる社内データを資料名・列・粒度まで指定する。", "06｜Day 3へ渡す：本日は証拠を強い／弱いと採点せず、出所・母集団・利害を評価できる状態に整える。"] },
    drills: [
    ...
    independent: { name: "B2B研修 · 失注レビュー", scene: "担当者は『内容は良いですが、価格が高いので検討します』と発言。提案後14日返信がない。営業はCRMに『価格で失注』と記録したが、失注理由の選択肢は一つしか選べない。", prompt: "発言・行動・営業の記録・原因仮説を分け、何をconfirmed／unverifiedとするか、価格原因を識別する次証拠まで示してください。", model: "Confirmed：見積提示後の発言原文と14日間返信なし。Unverified：価格が主因、案件が失注済み、内容評価が購買意思を示す、の3点。CRMの『価格で失注』は営業担当者の分類であり顧客事実ではない。次に、決裁者・予算承認日・比較提案・価格一定時の残余障害を確認し、価格変更前後または同条件案件の成約差で価格仮説を検査する。" },
    transfer: "Day 1で作った自案件のDecision Mandateから重要な主張を5つ抜き、意味分類／confirmed・contradicted・unverified／出所／適用範囲／次に取る証拠を一行ずつ記録する。",
    rubric: { pass: "観測・引用・計算と、解釈・因果仮説を別行にしている。", standard: "全行に証拠状態、出所、適用範囲があり、未確認を未確認のまま保てる。", pro: "意思決定を変える未確認仮説を優先し、部署・資料名・必要列・粒度まで次証拠を指定する。", revise: "『〜だから』『〜を望む』『〜が原因』を事実欄から外し、誰がどこで確認した何かへ戻す。" },
    output: "Decision-linked Evidence Separation Log", next: "証拠を分離しても、同じconfirmed情報でも母集団・測定方法・利害により重みは違う。Day 3で出所と証拠強度を評価する。", sources: [courseraSocial, zoom],
  },
  {
